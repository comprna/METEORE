...
```
* `example_nanopolish-freq-perCG.tsv` stores **the final per-site data** and provides the genomic position of the CpG site, the methylation frequency and the read coverage, which is in the same format generated by other tools. The methylation calls from both strands are merged into a single strand.
```
...
A Snakefile named `Deepsignal` contains all rules in the Snakemake workflow. Run it to create the output files:
```
snakemake -s Deepsignal deepsignal_results/example_deepsignal-freq-perCG-raw.tsv
```
This will produce the `deepsignal_results` output directory containing all output files, including:
...
```
conda deactivate
conda create -n meteore_deepsignal2_env -c bioconda -c conda-forge r-base=4.0.3 r-data.table r-dplyr
conda activate meteore_deepsignal2_env
```

```
* `example_deepsignal-freq-perCG.tsv` is the **final per-site results** containing the genomic position of the CpG site, methylation frequency and coverage, which is in the same format generated by other tools. The methylation calls from both strands are merged into a single strand, which requires the separate environment and Snakemake script below.
Chr             Pos         Methyl_freq     Cov
...
```
snakemake --cores 1 -s Deepsignal2 deepsignal_results/example_deepsignal-perRead-score.tsv
```
The output is in tsv format and contains four columns: `ID`,	`Pos`,	`Strand` and `Score`.
```
ID      Pos     Strand  Score
8386596c-ff56-4032-b54e-8f062c194b16    3507440 +       0.325419577989058
256a655c-b3f3-4131-805f-500052090c21    3500226 +       0.564751914888957
256a655c-b3f3-4131-805f-500052090c21    3500235 +       3.21037678690953
```
...
* `example_tombo-freq-perCG.tsv`: **the final per-site results** combining the methylation frequency and coverage results, which are in the same format generated by other tools
```
Chr             Pos         Methyl_freq     Cov
...
```
* `example_guppy-freq-perCG.tsv` contains the **per-site data** including the genomic position of the CpG site, methylation frequency and coverage, which is in the same format generated by other tools. The methylation calls from both strands are merged into a single strand.
```
Chr             Pos         Methyl_freq   Cov
...
```
megalodon data/example/ --outputs mods --reference data/ecoli_k12_mg1655.fasta --mod-motif m CG 0 --write-mods-text --processes 10 --guppy-server-path ./<path_to_ont-guppy-cpu>/bin/guppy_basecall_server --guppy-params "--num_callers 10" --guppy-timeout 400 --guppy-config res_dna_r941_min_modbases_5mC_CpG_v001.cfg --overwrite
```
...
* `example_megalodon-freq-perCG.tsv` contains the **per-site results** including the genomic position of the CpG site, methylation frequency and coverage, which is in the same format generated by other tools. The methylation calls from both strands are merged into a single strand.
