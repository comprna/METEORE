...
**Prepare the input file for combined model usage**
...
* `example_deepsignal-freq-perCG-raw.tsv` contains the default per-site results using a Python script provided by `DeepSignal`. The file contains 11 columns:
```
...
```
* `example_deepsignal-freq-perCG.tsv` also contains the per-site results but we combine the methylation calls from both strands into a single strand. The file contains the position of the CpG site on the reference genome, methylation frequency and coverage.
```
Chr             Pos       Methyl_freq     Cov
