...
```
* `example_deepsignal-freq-perCG.tsv` is the **final per-site results** containing the genomic position of the CpG site, methylation frequency and coverage, which is in the same format generated by other tools. The methylation calls from both strands are merged into a single strand.
```
Chr             Pos         Methyl_freq     Cov
